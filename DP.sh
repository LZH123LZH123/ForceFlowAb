...
Options:
  ...
  --cdr-lengths SPEC     Variable CDR lengths, e.g. H3:8-16 or H3:10|12|14,L3:9
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --cdr-lengths|--cdr_lengths) cdr_lengths="$2"; shift 2 ;;
...
"$PYTHON" - "$request_json" "$job_id" "$ab_type" "$region" "$pdb_path" "$heavy_chain" "$light_chain" "$batch_size" "$num_samples" "$cdr_lengths" "$job_dir" "$energy" "$energy_start" "$energy_end" "$energy_warmup" "$device" "$relax" "$relax_pipeline" "$relax_num_cpus" "$extra_tag" "$no_renumber" "$dry_run" <<'PY'
import json
import sys
from datetime import datetime
...
(
    ...
    num_samples, cdr_lengths, job_dir, energy, energy_start, energy_end, energy_warmup,
...
data = {
    ...
    "cdr_lengths": cdr_lengths or None,
    "job_dir": job_dir,
...
"$PYTHON" - "$base_config" "$run_config" "$num_samples" "$cdr_lengths" "$energy" "$energy_start" "$energy_end" "$energy_warmup" "$ab_type" "$region" "$region_cdrs" <<'PY'
import re
import sys
import yaml

base_config, run_config, num_samples, cdr_lengths, energy, start_step, end_step, warmup_steps, ab_type, region, region_cdrs = sys.argv[1:]
...
if cdr_lengths:
    aliases = {
        "H1": "H_CDR1", "H_CDR1": "H_CDR1",
        "H2": "H_CDR2", "H_CDR2": "H_CDR2",
        "H3": "H_CDR3", "H_CDR3": "H_CDR3",
        "L1": "L_CDR1", "L_CDR1": "L_CDR1",
        "L2": "L_CDR2", "L_CDR2": "L_CDR2",
        "L3": "L_CDR3", "L_CDR3": "L_CDR3",
    }
    parsed_lengths = {}
    for entry in cdr_lengths.split(","):
        if ":" not in entry:
            raise ValueError(
                "--cdr-lengths entries must use CDR:SPEC syntax, for example H3:8-16"
            )
        raw_name, raw_spec = entry.split(":", 1)
        name = raw_name.strip().upper().replace("-", "_")
        if name not in aliases:
            raise ValueError("Unsupported CDR in --cdr-lengths: {}".format(raw_name))
        canonical = aliases[name]
        if canonical not in cfg["sampling"]["cdrs"]:
            raise ValueError(
                "{} has a length specification but is not selected by --region".format(canonical)
            )
        spec = raw_spec.strip()
        if not re.fullmatch(r"\d+(?:\s*-\s*\d+)?|\d+(?:\s*[|/]\s*\d+)+", spec):
            raise ValueError("Invalid CDR length specification: {}".format(raw_spec))
        values = [int(value) for value in re.findall(r"\d+", spec)]
        if any(value < 5 or value > 30 for value in values):
            raise ValueError("CDR lengths must be between 5 and 30 residues")
        if "-" in spec and values[0] > values[1]:
            raise ValueError("CDR minimum length must not exceed maximum length")
        parsed_lengths[canonical] = spec
    cfg["sampling"]["cdr_lengths"] = parsed_lengths
...
echo "CDR lengths: ${cdr_lengths:-native}"
