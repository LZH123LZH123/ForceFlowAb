...
@register_transform('random_remove_antigen')
class RandomRemoveAntigen:

    def __init__(self, enabled=False, probability=0.1):
    ...
    def __call__(self, data):
        """Randomly remove antigen residues from an already merged/cropped patch."""
        if not self.enabled or random.random() >= self.probability:
            return data
        if 'fragment_type' not in data:
            raise KeyError(
                'random_remove_antigen must run after merge_chains and patch_around_anchor.'
            )

        antigen_mask = data['fragment_type'] == constants.Fragment.Antigen
        if not antigen_mask.any().item():
            return data
        return _mask_select_data(data, ~antigen_mask)
