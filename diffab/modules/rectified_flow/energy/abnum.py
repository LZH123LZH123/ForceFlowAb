...
def resolve_chain_types(structure_source, selected_chains=None):
    """Resolve antibody chains without re-running ANARCI when IDs are known."""
    model = _get_model(structure_source)
    selected_chains = _normalize_selected_chains(selected_chains)
    if selected_chains is None:
        return classify_chains(model)

    chain_ids = [chain.id for chain in model]
    heavy = selected_chains["heavy"]
    light = selected_chains["light"]
    if heavy is not None and light is not None and heavy == light:
        raise ValueError(
            "Energy guidance heavy and light chain IDs must be different; "
            f"both were {heavy!r}."
        )

    missing = [
        chain_id
        for chain_id in (heavy, light)
        if chain_id is not None and chain_id not in chain_ids
    ]
    if missing:
        raise ValueError(
            "Energy guidance antibody chain IDs were not found in the sampled "
            f"PDB: {missing}. Available chains: {chain_ids}."
        )

    return {
        chain_id: (
            "H" if chain_id == heavy
            else "L" if chain_id == light
            else "Ag"
        )
...
def reorder_and_renumber_pdb(structure_source, chain_map):
    ...
    available_chain_ids = string.ascii_uppercase + string.ascii_lowercase + string.digits
    if len(order) > len(available_chain_ids):
        raise ValueError(
            "Energy guidance supports at most "
            f"{len(available_chain_ids)} chains; got {len(order)}."
        )

    for new_id, old_id in zip(available_chain_ids, order):
...
def build_madrax_inputs(chain_map, reordered_model, renum_model, selected_cdrs=None):
    ...
    for renum_chain in renum_model:
        chain_id = renum_chain.id
...
def _preprocess_single_for_energy(args):  # 410
    ...
    chain_map = resolve_chain_types(source, selected_chains)
    ...
    if expected_cdr_count is not None and cdr_ca_count != int(expected_cdr_count):
        raise ValueError(
            "Energy-guidance CDR mapping mismatch for sample "
            f"{i}: sampled PDB contains {cdr_ca_count} selected CDR CA atoms, "
            f"but generate_flag contains {int(expected_cdr_count)} residues. "
            f"selected_chains={selected_chains}, selected_cdrs={selected_cdrs}."
        )
...
def flatten_guidance_vectors(grads_batch, h3_ranges=None, legacy_h3_only=False):
    """Flatten padded per-sample guidance in generate-mask traversal order."""
    if isinstance(grads_batch, dict):
        force = grads_batch.get("force", grads_batch.get("grad", None))
        torque = grads_batch.get("torque", None)
        counts = grads_batch.get("counts", None)
        if force is None:
            raise ValueError("grads_batch dict must contain 'force' or 'grad'.")
    elif isinstance(grads_batch, (tuple, list)) and len(grads_batch) == 2:
        force, torque = grads_batch
        counts = None
    else:
        force, torque = grads_batch, None
        counts = None
    ...
    elif not torch.is_tensor(torque):
        torque = torch.as_tensor(torque, device=force.device, dtype=force.dtype)
    ...
    if counts is not None:
        if torch.is_tensor(counts):
            ...
            counts = list(counts)
        if force.dim() != 3 or torque.dim() != 3:
            raise ValueError(
                "Count-aware energy guidance expects force and torque shaped "
                "(batch, max_selected_residues, 3)."
            )
        if force.size(0) != len(counts) or torque.size(0) != len(counts):
            raise ValueError(
                "Energy-guidance count metadata does not match the batch size."
            )
        ...
        for sample_idx, count in enumerate(counts):
            count = int(count)
            if count < 0 or count > force.size(1):
                raise ValueError(
                    f"Invalid energy-guidance residue count {count} for sample "
                    f"{sample_idx}; padded width is {force.size(1)}."
                )
            start, end = 0, count
            if legacy_h3_only:
                if h3_ranges is None or sample_idx >= len(h3_ranges):
                    raise ValueError("Legacy H3 guidance requires one H3 range per sample.")
                start, end = h3_ranges[sample_idx]
                if not (0 <= start <= end <= count):
                    raise ValueError(
                        f"Invalid H3 guidance range {(start, end)} for sample "
                        f"{sample_idx} with {count} selected residues."
                    )
            force_parts.append(force[sample_idx, start:end])
            torque_parts.append(torque[sample_idx, start:end])
        if not force_parts:
            return force.new_zeros((0, 3)), torque.new_zeros((0, 3))
        return torch.cat(force_parts, dim=0), torch.cat(torque_parts, dim=0)

    if legacy_h3_only:
        if force.dim() != 3 or h3_ranges is None:
            raise ValueError(
                "Legacy H3 guidance requires batched force tensors and H3 ranges."
            )
        force_parts = []
        torque_parts = []
        for sample_idx in range(force.size(0)):
            start, end = h3_ranges[sample_idx]
            force_parts.append(force[sample_idx, start:end])
            torque_parts.append(torque[sample_idx, start:end])
        return torch.cat(force_parts, dim=0), torch.cat(torque_parts, dim=0)

    return force.reshape(-1, 3), torque.reshape(-1, 3)
...
def run_energy_guidance(
    ...
    if expected_cdr_counts is not None:
        if torch.is_tensor(expected_cdr_counts):
            expected_cdr_counts = expected_cdr_counts.detach().cpu().tolist()
        else:
            expected_cdr_counts = list(expected_cdr_counts)
        if len(expected_cdr_counts) != batch_size:
            raise ValueError(
                "expected_cdr_counts must contain one value per sampled structure; "
                f"got {len(expected_cdr_counts)} values for batch size {batch_size}."
            )
    ...
    for i in range(batch_size):  # 410
        ...
        expected_cdr_count = (
            expected_cdr_counts[i] if expected_cdr_counts is not None else None
        )
    ...
    counts = [int(item["cdr_ca_count"]) for item in preprocessed]
    force_batch = torch.nn.utils.rnn.pad_sequence(
        [value.squeeze(0) for value in force_accum],
        batch_first=True,
    )
    torque_batch = torch.nn.utils.rnn.pad_sequence(
        [value.squeeze(0) for value in torque_accum],
        batch_first=True,
    )
    grads_batch = {
        "force": force_batch,
