...
def apply_sequence_initial_residue_prior(
    sequence_noise,
    sequence_init_aa,
    mask_generate,
    strength,
):
    """Shift generated sequence noise toward configured amino-acid types."""
    strength = float(strength)
    if sequence_init_aa is None or strength == 0.0:
        return sequence_noise
    if not math.isfinite(strength) or strength < 0.0:
        raise ValueError('sequence_init_strength must be finite and non-negative.')

    prior = clampped_one_hot(
        sequence_init_aa,
        num_classes=sequence_noise.size(-1),
    ).to(sequence_noise.dtype)
    valid_prior = (
        (sequence_init_aa >= 0)
        & (sequence_init_aa < sequence_noise.size(-1))
        & mask_generate.bool()
    )
    return sequence_noise + strength * prior * valid_prior[..., None]
