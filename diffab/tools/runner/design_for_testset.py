...
def get_energy_guidance_chains_for_variant(variant):
    data = variant['data']
    fragment_type = data['fragment_type'].tolist()
    chain_ids = data['chain_id']

    def _chain_for(fragment):
        matches = []
        for chain_id, fragment_value in zip(chain_ids, fragment_type):
            if fragment_value == int(fragment) and chain_id not in matches:
                matches.append(chain_id)
        if len(matches) > 1:
            raise ValueError(
                f"Expected one {fragment.name} chain for energy guidance, "
                f"but found {matches}."
            )
        return matches[0] if matches else None

    return {
        'heavy': _chain_for(Fragment.Heavy),
        'light': _chain_for(Fragment.Light),
    }
            ...
            if 'abopt' in config.mode:
                ...
                traj_batch = model.sample(batch, batch_id, data_cropped, data_variant=variant['data'], log_dir=log_dir,
                    ...
                    **get_energy_guidance_options(config),
                    'energy_guidance_chains': get_energy_guidance_chains_for_variant(variant),
